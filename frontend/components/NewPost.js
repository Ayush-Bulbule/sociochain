...
export default function NewPost() {
    ...
    async function uploadToIPFS() {
        try {
            ...
            const slicedContent = uploadData[0].replace("https://ipfs.thirdwebcdn.com/ipfs/", "");
    ...
    const newPost = async (e) => {
        e.preventDefault()
        await fetch('http://localhost:5000/api/savepost', {
            method: 'POST',
            headers: {
    ...
    return (
        <form onSubmit={newPost} className="fixed right-3 lg:w-[32%]  box-border my-3 bg-white p-4 rounded-lg shadow dark:bg-gray-900 ">
            ...
            <div className="flex py-2">
                <input type="file" onChange={e => setImgurl(e.target.files[0])} id="default-search" class="block w-full px-4 py-2 mr-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Enter url...." required />
                <button type="submit" class="text-white  bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 flex items-center justify-center">
                  Post
                </button>
            </div>
        </form>
