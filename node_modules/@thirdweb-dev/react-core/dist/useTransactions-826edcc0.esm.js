...
import { createAsyncLocalStorage, assertWindowEthereum, walletIds } from '@thirdweb-dev/wallets';
...
function ThirdwebWalletProvider(props) {
  ...
  const getWalletConfig = useCallback(wallet => {
    const id = wallet.walletId;

    // find in personal wallets if wrapper wallet
    if (activeWalletConfig && "config" in activeWalletConfig && activeWalletConfig.config.personalWallets) {
      // find in personal wallets
      return activeWalletConfig.config.personalWallets.find(w => w.id === id);
    }
...
function useThirdwebWallet() {
  ...
  if (!ctx) {
    throw new Error(`useThirdwebWallet() can only be used inside <ThirdwebProvider />`);
  }
  return ctx;
}
